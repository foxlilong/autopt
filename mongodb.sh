...
oneinstack_dir=/data/script/OneinStack
mongodb_ver=4.0.6
...
#DOWN_ADDR_MongoDB=http://mirrors.linuxeye.com/oneinstack/src
dbmongopwd=hahaps

Install_MongoDB() {

  pushd ${oneinstack_dir}/scr > /dev/null
  id -u mongod >/dev/null 2>&1
  [ $? -ne 0 ] && useradd -s /sbin/nologin mongod
  mkdir -p ${mongo_data_dir};chown mongod.mongod -R ${mongo_data_dir}
  [ ! -e ${oneinstack_dir}/${FILE_NAME} ] && wget -c --no-check-certificate ${DOWN_ADDR_MYSQL_BK}/${FILE_NAME};sleep 1;
...
Install_MongoDB 2>&1 | tee -a ${oneinstack_dir}/Install_MongoDB.log